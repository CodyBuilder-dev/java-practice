package com.example.customerbatch.repository;

import com.example.customerbatch.model.Customer;
import com.example.customerbatch.model.CustomerStatus;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.stereotype.Repository;

import java.time.LocalDateTime;
import java.util.List;
import java.util.UUID;

@Repository
public interface CustomerRepository extends JpaRepository<Customer, UUID> {

    // 특정 기간 이상 로그인하지 않은 활성 회원 조회
    @Query("SELECT c FROM Customer c WHERE c.status = :status AND c.lastLoginAt < :threshold")
    List<Customer> findByStatusAndLastLoginBefore(CustomerStatus status, LocalDateTime threshold);

    // 상태별 회원 조회
    List<Customer> findByStatus(CustomerStatus status);
}
